...
def generar_excel(declaracion: dict, contribuyente: dict) -> bytes:
    ...
    _hdr_full(ws, r,
              ...
              _DARK, size=12, height=24); r += 1
    _hdr_full(ws, r,
              f"FORMULARIO 210  —  AÑO GRAVABLE {año}  —  UVT: $ {UVT_2025:,.0f}",
              _DARK, size=11, height=20); r += 1
    ...
    _hdr_full(ws, r, "DATOS DEL DECLARANTE", _MED, height=16); r += 1
    _info(ws, r, "Nombre / Razón Social", nombre, _VLIGHT); r += 1
    _info(ws, r, "NIT / Cédula",          nit,    _WHITE);  r += 1
    _info(ws, r, "Ciudad",                ciudad, _VLIGHT); r += 1
    _info(ws, r, "Estado declaración",    est,    _WHITE);  r += 1
    _info(ws, r, "Período gravable",      año,    _VLIGHT); r += 1
    _blank(ws, r); r += 1

    # ── PATRIMONIO ────────────────────────────────────────────────────────────
    _hdr_full(ws, r, "PATRIMONIO", _MED); r += 1
    _pair(ws, r,
          29, "Patrimonio bruto",  v["c29"],
          30, "Deudas",            v["c30"]); r += 1
    _single(ws, r, 31, "Patrimonio líquido  (29 − 30)", v["c31"],
            bg=_LIGHT, bold_val=True); r += 1
    _blank(ws, r); r += 1

    # ── CÉDULA GENERAL ────────────────────────────────────────────────────────
    _hdr_full(ws, r, "CÉDULA GENERAL", _DARK, height=16); r += 1

    # Rentas de trabajo (relación laboral)
    _hdr_full(ws, r, "  RENTAS DE TRABAJO  (Art. 103 ET)", _MED, height=15); r += 1
    _pair(ws, r,
          32, "Ingresos brutos rentas de trabajo",               v["c32"],
          33, "Ingresos no constitutivos de renta — aportes pensión", v["c33"]); r += 1
    _single(ws, r, 34, "Renta líquida rentas de trabajo  (32 − 33)",
            v["c34"], bg=_VLIGHT, bold_val=True); r += 1
    _pair(ws, r,
          35, "Rentas exentas — AFC / FVP / AVC",           v["c35"],
          36, "Rentas exentas — 25%  núm. 10  Art. 206 ET", v["c36"]); r += 1
    _single(ws, r, 37, "Total rentas exentas rentas de trabajo  (35 + 36)",
            v["c37"], bg=_VLIGHT); r += 1
    _pair(ws, r,
          38, "Deducciones — Intereses crédito de vivienda", v["c38"],
          39, "Deducciones — Salud, dependientes y otras",   v["c39"]); r += 1
    _single(ws, r, 40, "Total deducciones imputables  (38 + 39)",
            v["c40"], bg=_VLIGHT); r += 1
    _single(ws, r, 41,
            "Rentas exentas y deducciones imputables LIMITADAS — 40% / 1.340 UVT  (Art. 336 ET)",
            v["c41"], bg=_LIGHT, height=18); r += 1
    _single(ws, r, 42, "Renta líquida ordinaria rentas de trabajo  (34 − 41)",
            v["c42"], bg=_LIGHT, bold_val=True); r += 1
    _blank(ws, r); r += 1
    ...
    _pair(ws, r, 43, "Ingresos brutos", 0,
                 44, "Ingresos no constitutivos de renta", 0); r += 1
    _single(ws, r, 53, "Renta líquida ordinaria  (46 − 51 − 52)", 0,
            bg=_VLIGHT); r += 1
    _blank(ws, r); r += 1

    # Rentas de capital (58-73)
    _hdr_full(ws, r, "  RENTAS DE CAPITAL  (Art. 58 ET)", _MED, height=15); r += 1
    _pair(ws, r,
          58, "Ingresos brutos rentas de capital",  v["c58"],
          59, "Ingresos no constitutivos de renta", 0); r += 1
    _pair(ws, r,
          60, "Costos y deducciones procedentes",    0,
          61, "Renta líquida  (58 − 59 − 60)",       max(0.0, v["c58"])); r += 1
    _pair(ws, r,
          65, "Total rentas exentas",                0,
          68, "Total deducciones imputables",         0); r += 1
    _single(ws, r, 73, "Renta líquida ordinaria rentas de capital",
            v["c58"], bg=_VLIGHT, bold_val=True); r += 1
    _blank(ws, r); r += 1

    # Rentas no laborales (74-90)
    _hdr_full(ws, r, "  RENTAS NO LABORALES  (Art. 74 ET)", _MED, height=15); r += 1
    _pair(ws, r,
          74, "Ingresos brutos rentas no laborales", v["c74"],
          75, "Devoluciones, rebajas y descuentos",  0); r += 1
    _pair(ws, r,
          76, "Ingresos no constitutivos de renta",  0,
          77, "Costos y deducciones procedentes",    0); r += 1
    _single(ws, r, 78, "Renta líquida  (74 − 75 − 76 − 77)",
            max(0.0, v["c74"]), bg=_VLIGHT); r += 1
    _pair(ws, r,
          82, "Total rentas exentas",     0,
          85, "Total deducciones imputables", 0); r += 1
    _single(ws, r, 90, "Renta líquida ordinaria rentas no laborales",
            v["c74"], bg=_VLIGHT, bold_val=True); r += 1
    _blank(ws, r); r += 1

    # Consolidado cédula general (91-97)
    _hdr_full(ws, r, "  CONSOLIDADO CÉDULA GENERAL", _MED, height=15); r += 1
    _single(ws, r, 91,
            "Renta líquida cédula general  (42 + 53 + 57 + 73 + 90)",
            v["c91"], bg=_VLIGHT, bold_val=True); r += 1
    _pair(ws, r,
          93, "Renta líquida ordinaria  (91 − 92)",          v["c91"],
          94, "Compensaciones por pérdidas año 2018 y ant.", 0); r += 1
    _pair(ws, r,
          95, "Compensaciones por exceso renta presuntiva",  0,
          96, "Rentas gravables",                             0); r += 1
    _single(ws, r, 97,
            "Renta líquida gravable cédula general  (93 + 96 − 94 − 95)",
            v["c97"], bg=_LIGHT, bold_val=True); r += 1
    _blank(ws, r); r += 1

    # ── CÉDULA DE PENSIONES (98-103) ──────────────────────────────────────────
    _hdr_full(ws, r, "CÉDULA DE PENSIONES  (Art. 337 ET)", _DARK, height=16); r += 1
    _pair(ws, r,
          99,  "Ingresos brutos por rentas de pensiones", 0,
          100, "Ingresos no constitutivos de renta",       0); r += 1
    _pair(ws, r,
          101, "Renta líquida  (99 − 100)",                0,
          102, "Rentas exentas de pensiones",              0); r += 1
    _single(ws, r, 103, "Renta líquida gravable cédula de pensiones  (101 − 102)",
            0, bg=_VLIGHT, bold_val=True); r += 1
    _blank(ws, r); r += 1
    ...
    _pair(ws, r,
          ...
          105, "Ingresos no constitutivos de renta",             0); r += 1
    _pair(ws, r,
          106, "Renta líquida ordinaria 2016 y ant.  (104−105)", 0,
          107, "1a. Subcédula 2017+ — núm. 3 Art. 49 ET",        v["c107"]); r += 1
    _pair(ws, r,
          108, "2a. Subcédula 2017+ — par. 2° Art. 49 ET",      0,
          109, "Dividendos y part. recibidos del exterior",      0); r += 1
    _blank(ws, r); r += 1

    # ── GANANCIAS OCASIONALES (112-115) ──────────────────────────────────────
    _hdr_full(ws, r, "GANANCIAS OCASIONALES  (Art. 299–316 ET)", _DARK, height=16); r += 1
    _pair(ws, r,
          112, "Ingresos por ganancias ocasionales del país y exterior", v["c115"],
          113, "Costos por ganancias ocasionales",                        0); r += 1
    _pair(ws, r,
          114, "Ganancias ocasionales no gravadas y exentas",  0,
          115, "Ganancias ocasionales gravables  (112−113−114)", v["c115"]); r += 1
    _blank(ws, r); r += 1

    # ── LIQUIDACIÓN PRIVADA ───────────────────────────────────────────────────
    _hdr_full(ws, r, "LIQUIDACIÓN PRIVADA", _DARK, height=16); r += 1
    _single(ws, r, 116, "Impuesto sobre renta cédula general  (Art. 241 ET)",
            v["c116"], bg=_VLIGHT, bold_val=True); r += 1
    _pair(ws, r,
          119, "Impuesto cédula dividendos 2016  (base cas. 106)", 0,
          120, "Impuesto cédula dividendos 2017 y siguientes",     0); r += 1
    _single(ws, r, 121,
            "Impuesto a cargo antes de descuentos  (116 + 119 + 120)",
            v["c116"], bg=_VLIGHT); r += 1
    _pair(ws, r,
          122, "Descuentos tributarios — impuestos pagados exterior",  0,
          123, "Descuentos tributarios — donaciones",                   0); r += 1
    _pair(ws, r,
          124, "Descuentos tributarios — dividendos y participaciones", 0,
          125, "Total descuentos tributarios  (122 + 123 + 124)",       0); r += 1
    _single(ws, r, 126, "Impuesto neto de renta  (121 − 125)",
            v["c116"], bg=_VLIGHT); r += 1
    _pair(ws, r,
          127, "Impuesto de ganancias ocasionales",                      v["c127"],
          128, "Descuento impuestos pagados exterior — gan. ocasionales", 0); r += 1
    _single(ws, r, 129, "Total impuesto a cargo  (126 + 127 − 128)",
            v["c129"], bg=_LIGHT, bold_val=True); r += 1
    _blank(ws, r); r += 1

    # Anticipo y retenciones
    _hdr_full(ws, r, "  ANTICIPO Y RETENCIONES", _MED, height=15); r += 1
    _pair(ws, r,
          130, "Anticipo renta liquidado año gravable anterior", 0,
          131, "Saldo a favor año gravable anterior",            0); r += 1
    _pair(ws, r,
          132, "Retenciones año gravable a declarar",            v["c132"],
          133, "Anticipo renta para el año gravable siguiente",  0); r += 1
    _blank(ws, r); r += 1

    # Saldo a pagar / a favor
    _hdr_full(ws, r, "  SALDO A PAGAR / SALDO A FAVOR", _MED, height=15); r += 1
    _pair(ws, r,
          134, "Saldo a pagar por impuesto  (129+133−130−131−132)", v["c134"],
          135, "Sanciones",                                          0); r += 1
    _single(ws, r, 136,
            "Total saldo a pagar  (129 + 133 + 135 − 130 − 131 − 132)", v["c136"],
            bg=_BG_RED if v["c134"] > 0 else _WHITE, bold_val=True); r += 1
    _single(ws, r, 137,
            "Total saldo a favor  (130 + 131 + 132 − 129 − 133 − 135)", v["c137"],
            bg=_BG_GREEN if v["c137"] > 0 else _WHITE, bold_val=True); r += 1
    _blank(ws, r); r += 1

    # ── DATOS ADICIONALES ─────────────────────────────────────────────────────
    _hdr_full(ws, r, "DATOS ADICIONALES", _MED, height=15); r += 1
    ...
    _pair(ws, r,
          ...
          139, "Adición por dependientes a casilla 92",       0); r += 1
    _pair(ws, r,
          140, "Superó tope indicativo Art. 336-1 ET (X=sí)", "—",
          141, "Tipo de ganancia ocasional",                  tipo_gan); r += 1
    _blank(ws, r); r += 1
