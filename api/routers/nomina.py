...
async def exportar(
    ...
    tipo_label = "Nómina Mensual" if tipo == "mensual" else "Liquidación Definitiva"
    sub = ws.cell(
        row=2,
        column=1,
        value=f"{tipo_label} · SMLMV $1,750,905 · Aux.Transporte $249,095",
    )
    ...
    if tipo == "mensual":
        ...
        section(row, "▸ DEVENGADO")
        row += 1
        row_data(row, "Salario proporcional", r.salario_proporcional)
        row += 1
        if r.aplica_aux_transporte:
            row_data(row, "Auxilio de transporte", r.auxilio_transporte)
            row += 1
        he_labels = [
        ...
        for label, val in he_labels:
            if val:
                row_data(row, f"  · {label}", val)
                row += 1
        if r.bonificaciones:
            row_data(row, "Bonificaciones salariales", r.bonificaciones)
            row += 1
        if r.comisiones:
            row_data(row, "Comisiones", r.comisiones)
            row += 1
        if r.otros_no_salariales:
            row_data(row, "Conceptos no salariales", r.otros_no_salariales)
            row += 1
        row_data(row, "TOTAL DEVENGADO", r.total_devengado, bold=True)
        row += 2

        section(row, "▸ DEDUCCIONES EMPLEADO")
        row += 1
        row_data(row, "IBC – Base Seg. Social", r.ibc)
        row += 1
        row_data(row, "Salud empleado (4%)", r.salud_empleado)
        row += 1
        row_data(row, "Pensión empleado (4%)", r.pension_empleado)
        row += 1
        if r.fsp:
            row_data(row, "FSP – Fondo Solidaridad Pensional", r.fsp)
            row += 1
        if r.retencion_fuente:
            row_data(row, "Retención en la fuente", r.retencion_fuente)
            row += 1
        if r.otras_deducciones:
            row_data(row, "Otras deducciones", r.otras_deducciones)
            row += 1
        row_data(row, "TOTAL DEDUCCIONES", r.total_deducciones, bold=True)
        row += 2
        ...
        section(row, "▸ CARGA PATRONAL – PARAFISCALES Y SEGURIDAD SOCIAL")
        row += 1
        row_data(row, "Salud empleador (8.5%)", r.salud_empleador)
        row += 1
        row_data(row, "Pensión empleador (12%)", r.pension_empleador)
        row += 1
        row_data(row, f"ARL – {ARL_LABELS.get(arl_clase, ARL_LABELS[1])}", r.arl)
        row += 1
        row_data(row, f"SENA (2%){exon_txt}", r.sena)
        row += 1
        row_data(row, f"ICBF (3%){exon_txt}", r.icbf)
        row += 1
        row_data(row, "Caja de Compensación Familiar (4%)", r.caja_compensacion)
        row += 1
        row_data(row, "TOTAL CARGA PATRONAL", r.total_carga_patronal, bold=True)
        row += 2
        ...
        section(row, "▸ DATOS DEL CONTRATO")
        row += 1
        row_data(row, "Fecha de ingreso", str(body.get("fecha_ingreso", "")))
        row += 1
        row_data(row, "Fecha de retiro", str(body.get("fecha_retiro", "")))
        row += 1
        row_data(row, "Días trabajados (base 360)", r2.dias_trabajados)
        row += 1
        row_data(row, "Tipo de contrato", body.get("tipo_contrato", ""))
        row += 1
        row_data(row, "Causa terminación", body.get("causa_terminacion", ""))
        row += 2

        section(row, "▸ PRESTACIONES SOCIALES")
        row += 1
        row_data(row, f"Cesantías ({r2.dias_cesantias} días)", r2.cesantias)
        row += 1
        row_data(row, "Intereses sobre cesantías (12%/360)", r2.intereses_cesantias)
        row += 1
        row_data(row, f"Prima de servicios ({r2.dias_prima} días)", r2.prima_servicios)
        row += 1
        row_data(
            row,
            f"Vacaciones ({
                round(
                    r2.dias_vacaciones_a_pagar,
                    1)} días pendientes)",
            r2.vacaciones_proporcionales)
        row += 1
        row_data(row, "Salario pendiente último período", r2.salario_pendiente)
        row += 1
        if r2.indemnizacion:
            row_data(row, f"Indemnización · {r2.causa_indemnizacion}", r2.indemnizacion)
            row += 1
        row_data(row, "SUBTOTAL DEVENGADO", r2.subtotal_devengado, bold=True)
        row += 1
        if r2.otras_deducciones:
            row_data(row, "(-) Deducciones / préstamos", -r2.otras_deducciones)
            row += 1
        row += 1
    ...
    row_data(row, "Generado por TaxOps SaaS", f"Fecha: {date.today().isoformat()}")
    row += 1
...
async def exportar_pdf(
    ...
    if tipo == "mensual":
        ...
        for lbl, v in he_map:
            if v:
                dev_rows.append([f"  · {lbl}", cop(v)])
        if r.bonificaciones:
            dev_rows.append(["Bonificaciones salariales", cop(r.bonificaciones)])
        if r.comisiones:
            dev_rows.append(["Comisiones", cop(r.comisiones)])
        if r.otros_no_salariales:
            dev_rows.append(["No salariales", cop(r.otros_no_salariales)])
        ...
        ded_rows = [["Concepto", "Valor"],
                    ["IBC – Base Seg. Social", cop(r.ibc)],
                    ["Salud empleado (4%)", cop(r.salud_empleado)],
                    ["Pensión empleado (4%)", cop(r.pension_empleado)]]
        if r.fsp:
            ded_rows.append(["FSP – Solidaridad Pensional", cop(r.fsp)])
        if r.retencion_fuente:
            ded_rows.append(["Retención en la fuente", cop(r.retencion_fuente)])
        if r.otras_deducciones:
            ded_rows.append(["Otras deducciones", cop(r.otras_deducciones)])
        ded_rows.append(["TOTAL DEDUCCIONES", cop(r.total_deducciones)])
        story.append(make_table(ded_rows))

        # Neto highlight
        story.append(Spacer(1, 0.2 * cm))
        neto_t = Table([["NETO A PAGAR", cop(r.neto_pagar)]], colWidths=[10 * cm, 4.5 * cm])
        ...
        pres_rows = [
            [
                "Concepto", "Valor"], [
                f"Cesantías ({
                    r2.dias_cesantias} días)", cop(
                    r2.cesantias)], [
                        "Intereses sobre cesantías (12%)", cop(
                            r2.intereses_cesantias)], [
                                f"Prima de servicios ({
                                    r2.dias_prima} días)", cop(
                                        r2.prima_servicios)], [
                                            f"Vacaciones ({
                                                round(
                                                    r2.dias_vacaciones_a_pagar, 1)} días pendientes)", cop(
                                                        r2.vacaciones_proporcionales)], [
                                                            "Salario pendiente", cop(
                                                                r2.salario_pendiente)]]
        if r2.indemnizacion:
...
async def import_batch(
    ...
    for _, row in df.iterrows():
        try:
            salario = float(str(row.get("salario_basico", 0)).replace(
...
async def colilla_pdf(periodo: str, empleado_id: str, user: dict = Depends(get_current_user)) -> StreamingResponse:
    ...
    if dev.get("bonificaciones"):
        dev_rows.append(["Bonificaciones salariales", cop(dev["bonificaciones"])])
    if dev.get("comisiones"):
        dev_rows.append(["Comisiones", cop(dev["comisiones"])])
    if dev.get("otros_no_salariales"):
        dev_rows.append(["No salariales", cop(dev["otros_no_salariales"])])
    ...
    if ded.get("fsp"):
        ded_rows.append(["FSP – Solidaridad", cop(ded["fsp"])])
    if ded.get("retencion_fuente"):
        ded_rows.append(["Retención en la fuente", cop(ded["retencion_fuente"])])
    if ded.get("otras"):
        ded_rows.append(["Otras deducciones", cop(ded["otras"])])
...
async def calcular_prima_batch(semestre: str, body: dict, user: dict = Depends(get_current_user)) -> dict:
    ...
    fecha_pago_limite = ""
    if resultados and "fecha_pago_limite" in resultados[0]:
        fecha_pago_limite = resultados[0]["fecha_pago_limite"]
...
async def _generar_colilla_pdf(resultado: dict, nombre: str, periodo: str) -> bytes:
    ...
    ded_rows = [["Concepto", "Valor"],
                ...
                ["Pensión empleado (4%)", cop(ded.get("pension_empleado", 0))]]
    if ded.get("fsp"):
        ded_rows.append(["FSP", cop(ded["fsp"])])
    if ded.get("retencion_fuente"):
        ded_rows.append(["Retención fuente", cop(ded["retencion_fuente"])])
    ded_rows.append(["TOTAL DEDUCCIONES", cop(ded.get("total_deducciones", 0))])
