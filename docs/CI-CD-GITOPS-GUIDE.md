...
| **Rol de CI acotado (no `AdministratorAccess`)** | HECHO (2026-09-16): tres roles OIDC — `plan` (pull_request, ReadOnly + lock), `terraform` (environment production, PowerUser + IAM acotado a `taxops-*`), `deploy` (main, ECR + UpdateFunctionCode + S3 config). Ver `infra/modules/github-oidc/main.tf`. | — |
